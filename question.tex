...
Since $F_{h_n}$ and $\ZZ[BG]$ are fibrant we can represent an element of $\pi_1(F_{h_n}, 0)$ with 
 a pair $(g, s) \in G^{2n} \times \ZZ[BG]_2$ satisfying $d_2(s) = h_n(g)$. 
%It remains to find in $F_{h_n}$ the 1-simplex homotopic to the path $\phi(\gamma_{x,y})$ (cf.~\cref{ex44}).
It is easy to check that the image of the loop $p_{ij}(x)$ under the natural map $T \to F_{h_n}$ is represented by the pair
$((e, d_{ij}(x)), (x, x^{-1}) + (e, e) - (x, e) - (e, x^{-1})). $

Filler can be computed as follows: $s_0 p_0 - s_2 d_3 s_0 p_0 + s_2 p_3 - s_1 d_2 s_0 p_0 + s_1 d_3 s_0 p_0 - s_1 p_3 + s_1 p_2$.
Formula for composante: $p_0 + p_2 - p_3 - s_1 d_2 p_0  + s_1 d_1 p_3 + s_0 d_2 p_0$
