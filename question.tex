...
\title[A note on amalgams of symmetric groups]{A note on amalgams of symmetric groups and \\ nonabelian tensor products}
...
Denote by $S_n(G)$ the quotient of $F$ modulo relations $s_{(g)}$ = $s_{(h)}$ in which $g, h \in G^n$ and $s\in S_n$
 are such that equality $s^g = s^h$ holds in the wreath product $G \wr S_n = G^n \rtimes S_n$
