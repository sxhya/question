...
Let $G$ be a group. The nonabelian tensor square $G \otimes G$ of $G$ is an extension of the derived subgroup $[G, G]$ that was defined by ??? in ???.
In this paper we will be mainly interested in the following two extensions of $[G, G]$ that are quotients of $G\otimes G$: the nonabelian exterior product $G \wedge G$ and
...
By definition, an {\it $\mathfrak{H}_n$-extension} is an extension $H$ of the group $D_n(G)$ that fits into the following pullback square
 for $U = G \mathbin{\widetilde{\wedge}} G, G \wedge G.$
\[ \xymatrix{ U \pullbackcorner \ar@{^{(}->}[d] \ar@{->>}[r] & [G, G] \ar@{^{(}->}[d]^{\iota_k} \\ H \ar@{->>}[r] & D_n(G)} \]
Extensions of type $H_n(G)$ formalize the concept of the group of noncommutative Steinberg symbols and the purpose of their original invention
 was to formulate the Statement of the noncommutative version of Matsumoto theorem.
 first m
