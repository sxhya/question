...
% (the reader is advised to draw a triangular prism at this point).
...
Using the above formula we get that the image of $c_{ij}(u,v)$ via $\varphi\beta$ equals $((e, e), t)$, where
...
It it not hard to check that $t-c'$ is the image under the differential $d_0-d_1+d_2-d_3$ of the following $3$-cycle:
\[(xy, x^{-1}, y^{-1}) - (y, x, x^{-1}) + s_1 s_1(x^{-1} + xy - x - y) + s_0 s_0(x^{-1} - x - x^{-1}y^{-1}).\]
%(x^{-1}, e, e) + (xy, e, e) - (x, e, e) - (y, e, e) - (e, e, x^{-1}y^{-1}) + (e, e, x^{-1}) - (e, e, x).\]
...
Since $\psi$ is natural in $G$, by~\cref{lm:endotr} below we get that $\psi$ is an isomorphism for arbitrary $G$,
 therefore $\phi$ is also an isomorphism, as claimed. \end{comment}
\end{proof}
