...
In order to do this we use a simplicial version of the so-called ''weak covering homotopy property``, cf.~\cite[\S~1.4.4]{KSz14}
 (also called ''delayed homotopy lifting property`` in the literature).

\begin{df} \label{df:dhlp}
 Let $C$ be a subclass of cofibrations in the category of simplicial sets.
...
Denote by $C_{pr}$ the class consisting of all the inclusions $i\colon U\hookrightarrow V$ of finite polyhedral simplicial sets 
 posessing a retract $r\colon V\to U$.  
\begin{lemma} \label{lm:dhlp} Let $X$ be a fibrant connected simplicial set.
The map $h_\infty \colon X^\infty \to \ZZ[X]$ satisfies the weak covering homotopy property with respect to the class $C_{pr}$.
\end{lemma}
\begin{proof}
 Notice that the problem can be reduced to the special case $g=0$.
 Since $U$ is finite the image of $g$ is contained in $X^{2N} \subset X^\infty$.
 Assume that $\widetilde{F}_0$ is a lifting for the diagram~\eqref{eq:plp-def} in which the top map equals $0$ and the bottom is $F_0 = F - h_\infty gr$.
 For any maps $f\colon A \to X^\infty$ and $g\colon A \to X^{2N}$ one can define their product $g\times f$ by $(g \times f)(a) = (g(a); f(a)) \in X^\infty$.  
 Clearly, then $\widetilde{F} = gr \times \widetilde{F}_0$ would be the desired lifting of $F$.
 ...
 Since $p \colon W \twoheadrightarrow X$ is surjective, the associated map between free simplicial abelian groups is a Kan fibration
  and we can choose a lifting $\widetilde{F}$ in the following diagram
\begin{equation} \nonumber \xymatrix{ 
U \ar[r]^{0}  \ar@{^{(}->}_{i_0}[d] & \ZZ[W] \ar@{->>}[d]  \\
V \ar[r]_{F} \ar[ru]_{\widetilde{F}} & \ZZ[X]}
\end{equation}
 %It is clear that $F$ has the desired lifting if so does $\widetilde{F}$  a lift in the diagram \eqref{eq:W-lift} below implies the existence of a lift for the original diagram with $F$ and $h_\infty$.

 By~\cite[Lemma~9.1]{Po17} the map $\widetilde{F}$ can be lifted along $h_W$ in such a way that the bottom triangle in following diagram commutes strictly.
...
\end{equation}
 ...
 It is not hard to show that $h_W^{-1}(0)$ is contractible (the contracting homotopy for $W^\infty$ can be restricted to $h_W^{-1}(0)$).
...
\begin{lemlist}
 ...
 \item Two diagrams $(a_0, A_0)$ and $(a_1, A_1)$ represent the same element of $G_k$ iff there exists a family of maps $(a_t, A_t)$
     continously depending on $t\in [0, 1]$ such that $pa_t = A_t i_0$ holds for all $t$ (we call such family a homotopy of diagrams).
 \item If $A\pi$ lifts with respect to $p$ %(i.\,e. there is $\widetilde{A}$ such that $\widetilde{A} \iota_0 = a$ and $p\widetilde{A}=A$)
...
\begin{proof}
 ...
 This is accomplished in a series of steps by means of the simplicial approximation theorem~\cite[Theorem~4.7]{Jar04}.
 For brevity we denote the subdivision functor $\sd^m\sd_*(-)$ by $s^m(-)$ and denote the canonical natural transformation $s^m X \to X$ by $\delta^m$
  (instead of $\gamma_*\gamma^m$).
 \begin{itemize}
  ...
  \item Invoke the approximation theorem once again (with the initial condition specified by $|a'_0|$) and find $q>m$ and $a''\colon s^{q}(D_k) \to E$
    such that $a'|\delta^{q}| \cong |a''|$ rel $S^k$.
  \item Using \cref{item:continue} we extend the homotopy $a \cong a' \cong a''$ to a homotopy of diagrams $(a, A) \cong (a'', A')$.
 ...
 Thus, we have obtained the following commutative diagram of simplicial sets
   ...
   \end{equation}
 By the functoriality of simplicial approximation the map $s^{r}(i)$ is retractible hence
  by the assumption of the lemma there is a lifting $\widetilde{A}$ satisfying the assumptions of~\cref{item:weaker}.
 \end{proof}
