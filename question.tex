...
Its $k$-simplices $W_k$ are matrices $\left(\begin{smallmatrix}x_0 & x_1 & \ldots & x_k&\\ n_0 & n_1 & \ldots & n_k \end{smallmatrix}\right)$,
 ...
 In view of Quillen theorem A (cf.\cite[ex.~3.11, p.~325]{Kbook}) it suffices to show that for each $p$-simplex $d \colon \Delta^p \to V$ the 
...
Factor the map $h_n$ as a composition of a trivial cofibration followed by a fibration: $BG^{2n} \hookrightarrow E_{h_n} \twoheadrightarrow \ZZ[BG]$
 and write down the starting portion of the long homotopy exact sequence of $E_{h_n} \twoheadrightarrow \ZZ[BG]$.
...
Thus, to finish the proof of~\cref{thm:main} it remains to show that $\psi$ is an isomorphism. 

Notice first that the map $\psi$ is natural in $G$. 
This follows from the fact that the isomorphisms $K \cong \HH_2(G, \ZZ)$ and the maps $\phi$ can be defined naturally with respect to $G$.
For example, in order to choose $E_{h_n}$ and $F_{h_n}$ naturally in $G$ we can use the usual path space construction
...
\[ \xymatrix{ E_{h_n}  \ar[r]_{\pi_2} \ar[d]_{\pi_1} \pullbackcorner & \ar[d]^{ev_0} \ZZ[BG]^I\\
   ...
   \xymatrix{ F_{h_n}  \ar[r] \ar[d] \pullbackcorner & \ar[d]^{ev_1\,\circ\,\pi_2} E_{h_n} \\
              pt \ar[r]_{0} & \ZZ[BG]}  \]
It is possible to classify all the natural transformations $\HH_2(G, \ZZ) \to \HH_2(G, \ZZ)$~see Lemma~{\bf ???} below.
As a matter of fact, such maps correspond to multiplication by some integer $n$.
Thus, it remains 
\end{proof}

\begin{lemma} The natural transformations $\HH_2(G, \ZZ) \to \HH_2(G, \ZZ)$ of the second group homology functor 
 $\HH_2(-, \ZZ)\colon \catname{Groups}\to \catname{Ab}$ coincides morphisms of multiplication by $n$.
