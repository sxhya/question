...
The aim of this paper is to obtain a new presentation for several extensions of type $\mathfrak{H}_n(G)$ and thereby
 to establish a surprising connection between symmetric groups and noncommutative symbols.
These presentations are then used to study homotopy invariants of infinite symmetric products {\bf ???}.
...
The universal {\it extension of type $\mathfrak{H}_n$} $H_n(G)$ and its antisymmetrization $H^\wedge_n(G)$ are given by generators and relations (see ???).
These extensions are the natural extensions of $D_n(G)$ that fit into the following pullback squares:
\[ \xymatrix{ G \mathbin{\widetilde{\wedge}} G \pullbackcorner \ar@{^{(}->}[d] \ar@{->>}[r] & [G, G] \ar@{^{(}->}[d]^{\iota_k} \\ H_n(G) \ar@{->>}[r] & D_n(G)} \]
\[ \xymatrix{ G \wedge G \pullbackcorner \ar@{^{(}->}[d] \ar@{->>}[r] & [G, G] \ar@{^{(}->}[d]^{\iota_k} \\ H^\wedge_n(G) \ar@{->>}[r] & D_n(G)} \]
Here $G \wedge G$ denotes the nonabelian exterior product and $G \mathbin{\widetilde{\wedge}} G$ is the modern notation for the Dennis functor $(G, G)$, see ??? (also denoted $U_G$ by Rehmann in ???). 
The map $\iota_k$ is the inclusion map that puts an element of $[G,G]$ on $k$-th position.
 

\begin{comment}
The main use of extensions of type $H_n(G)$ is that they formalize the concept of the group of noncommutative Steinberg symbols, see ???.
The goal of their original invention was to provide the framework for the noncommutative version of Matsumoto theorem.

More precisely, we will be interested in the so-called {\it extensions of type $\mathfrak{H}_n(G)$} associated to these extensions. 
 
Let $G$ be a group. Recall that the nonabelian tensor square $G \otimes G$ of $G$ 
is an extension of the derived subgroup $[G, G]$ that was defined by ??? in ???.
In this paper we will be mainly interested in the following two extensions of $[G, G]$ 
 that are quotients of $G\otimes G$: the nonabelian exterior product $G \wedge G$ and
 its extension $G \mathbin{\widetilde{\wedge}} G$ defined by Dennis in ???.
 
 
For the first time they have appeared in U.~Rehmann's paper~\cite{Reh78}, 
 where they are used as a noncommutative analogues in the proof of the noncommutative version of Matsumoto's theorem.
 
Discovery of noncommutative Steinberg symbols in the proof of the latter led to the
 definition of noncommutative tensor and exterior products in~\cite{De76}. 
\end{comment}
 
\begin{thm} \label{thm:summary} Blah-blah-blah \end{thm}

The presentation of Theorem 1 was discovered in attempt to solve some purely topological problem.
...
\begin{lemma} Let $U$ be a compact polyhedral {\bf ???} simplicial set, $X$ be a fibrant connected simplicial set and $F$, $g$ be arbitrary maps. 
There exists $\widetilde{F}$ making the following diagram commute.
\begin{equation} \nonumber \xymatrix{ U \ar[rr]^{g}  \ar_{\iota_0}[d] & & X^{\infty} \ar_{h_\infty}[d]  \\
                           U \times [0,2] \ar[r]_{id\times \tau} \ar@{-->}[rru]^{\widetilde{F}} & U \times [1,2] \ar[r]_{F} & \mathbb{Z}[X]}
\label{eq:plp} \end{equation}
Here we denote by $\tau$ the simplicial map $[0,2] \to [1,2]$ sending $0,1 \mapsto 1$ and $2 \mapsto 2$.
\end{lemma}
