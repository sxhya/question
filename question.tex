...
\end{lemma}
\begin{proof}
 Notice first that the problem can be reduced to the special case when $g=0$.
 Since $U$ is finite the image of $g$ is contained in $X^{2N} \subset X^\infty$.
 Assume that $\widetilde{F}_0$ is a lift for the diagram~\eqref{eq:plp-def} in which the top map equals $0$ and the bottom is $F_0 = F - h_\infty gr$.
 For any maps $f\colon A \to X^\infty$ and $g\colon A \to X^{2N}$ one can define the product map $g\times f$ by $(g \times f)(a) = (g(a); f(a)) \in X^\infty$.  
 Clearly, then $\widetilde{F} = gr \times \widetilde{F}_0[2N]$ would be a solution to the original lifting problem for $g$ and $F$. 
...
\end{equation}
 Obviously, if there exists a lift $G$ for the diagram \eqref{eq:W-lift} below then there is a lift for the original diagram (e.\,g. the diagram with $F$ and $h_\infty$).

 By~\cite[Lemma~9.1]{Po17} we can lift $\widetilde{F}$ along $h_W$ so that the bottom triangle in the diagram
\begin{equation} \label{eq:W-lift} \xymatrix{ 
U \ar[r]^{0}  \ar@{^{(}->}_{i_0}[d]                               & W^\infty \ar[d]^{h_W}  \\
V \ar[r]_{\widetilde{F}} \ar@{-->}[ru]^{G} & \ZZ[W]}
\end{equation}
 commutes strictly. It is clear that the image of $Gi_0$ is contained in the fiber $h_W^{-1}(0)$.
 Since $h_W^{-1}(0)$ is contractible (the contracting homotopy for $W^\infty$ can be restricted to $h_W^{-1}(0)$)
  we get that $Gi_0$ is fiberwise homotopic to $0$. \end{proof}
...
\begin{proof}
 ...
 The idea of the proof is to replace $(a, A)$ with a homotopic pair of simplicial maps and then invoke the weak covering homotopy property.
 ...
 \begin{itemize}
  \item Denote the restriction of $a$ to $S^k$ by $a_0$.
   ...
   Using the simplicial approximation theorem~\cite[Theorem~4.7]{Jar04}
  ...
  \item It is clear that there is a map $a'\colon D^k\to |E|$ extending $|a'_0|$ that is fiberwise homotopic to $a$.
  ...
  \item Applying \cref{item:continue} we extend the homotopy $a \cong a' \cong a''$ to a homotopy of diagrams $(a, A) \cong (a'', A')$.
  \item Applying the approximation theorem to $A'$ with the initial condition on the boundary $\partial (D_k \times I) = D^k\times \{0\} \cup J$ given by 
    $pa''$ and $const_b$, respectively, we find an integer $r>q$ and a map $A''\colon \sd^{r}(D_k \times I) \to B$ such that
    $A'|\gamma^r|\cong |A''|$ rel $\partial (D_k \times I)$.
 \end{itemize}
 Thus, we have obtained the following commutative diagram of simplicial sets
   ...
   \end{equation}
 By functoriality of $\sd$ the map $\sd^{r}(i)$ is retractible and
  by the assumption of the lemma there exists a lift $\widetilde{A}$ satisfying the assumptions of~\cref{item:weaker}.
 \end{proof}
