...
\begin{lemlist}
 ...
 \item \label{item:weaker} The claim of the third assertion remains true if we require the weaker condition $\widetilde{A}\iota_0\cong a$ instead of the corresponding strict equality.
\end{lemlist}
\end{lemma}
...
\begin{lemma} \label{lm:weak-equiv} If a map $p\colon E \to B$ possesses the delayed homotopy lifting property with respect to $C_{pr}$ 
 then its geometric realization is a quasifibration, i.\,e. for every point $b$ of $B$ the inclusion $p^{-1}(b) \hookrightarrow F_{p}(b)$ is a weak equivalence. \end{lemma}
\begin{proof}

 It suffices to show that $\pi_k(F_{p}(b), p^{-1}(b))$ are all trivial for $k \geq 1$.
 By~\cref{item:weaker} it suffices to show the existence of lifting $\widetilde{A}$ for every pair of maps $(a, A)$ in the following diagram:
 \begin{equation} \nonumber \xymatrix{ 
 D^k          \ar[r]^{a}  \ar@{^{(}->}_{\iota}[d] & E \ar@{->>}[d]  \\
 D^k \times I \ar[r]_{A}  \ar@{-->}[ru]_{\widetilde{A}} & B}
\end{equation}
