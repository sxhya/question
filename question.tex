...
\begin{lemma} Let $p\colon (E, e) \to (B, b)$ be a map of based topological spaces.
...
\begin{enumerate}
 \item Maps of triples $(D^k, S^k, pt) \to (F_p(b), p^{-1}(b), e)$ are in one-to-one correspondece with pairs of maps $(a, A)$ making the following diagram commute.
   \begin{equation} \nonumber \xymatrix{ 
    D^k \ar@{^{(}->}_{\iota_0}[d]  \ar_{a}[rr] & & E \ar^{p}[d]  \\
    D^k\times I \ar^{\pi}[r] & (D^k \times I) / J \ar^(.7){A}[r] & B}
    \label{eq:plp} \end{equation}
    Here $J$ denotes $\partial (D^k \times I) \cup (D^k \times \{1\})$.
 \item Two pairs $(a_0, A_0)$ and $(a_1, A_1)$ represent the same element of $G_k$ iff there exists a family of maps $(a_t, A_t)$
     continously depending on $t\in [0, 1]$ so that $pa_t = A_t\iota_0$ holds for all $t$ (we call such family a homotopy of pairs).
 ...
 \item Let $(a, A)$ be a pair as above and $H$ be a homotopy between two maps $a, a'\colon D^k \to E$ relative to $S^k$. 
       Then there exists a map $A'$ such that $(a', A')$ is a pair homotopic to $(a, A)$.
 \item The claim of the third assertion remains true if we require the weaker condition $\widetilde{A}\iota_0\cong a$ instead of the corresponding strict equality.
\end{enumerate}
\end{lemma}


\begin{lemma} \label{lm:weak-equiv} If a map $p\colon E \to B$ possesses the delayed homotopy lifting property with respect to $C_{pr}$ 
 then its geometric realization is a quasifibration, i.\,e. for every point $b$ of $B$ the inclusion $p^{-1}(b) \hookrightarrow F_{p}(0)$ is a weak equivalence. \end{lemma}
\begin{proof}
 It suffices to show that all relative homotopy groups $\pi_k(F_{p}(b), p^{-1}(b))$, $k \geq 1$ are trivial.
