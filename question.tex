...
\title[Amalgams of symmetric groups]{Amalgams of symmetric groups and \\ nonabelian tensor products}
...
\begin{itemize}
 \item The presentation is similar in spirit to results of Brown---Loday and Kassel---Reutenauer on the presentation of the linear Steinberg group in the form of an amalgam of braid groups.
 \item It gives a presentation of the universal extension $H_n(G)$ with generators and relations which is simpler than the original presentation given in~\cite{Reh78}, see~\cref{prop:simpler}.
       Moreover, our presentation explicitly obtained from the presentation of $S_n$ formulated in terms of transpositions.
\end{itemize}


%The proof of Theorem 1 proceeds as follows: 
%The presentation of Theorem is similar in spirit (albeit simpler) than the presentation
...
%In this section we obtain an explicit presentation of $S_n(G)$ similar in appearance . % by means of generators and relations.
...
The following lemma provides a more concrete description of the isomorphism $\theta$ from~\cref{cor:ker-iso}.
\begin{lemma} \label{lem:concrete-formula}
 ...
 The latter is defined as the following concatenation of loops in $T$:
 \[f_{ij}(u,v) = p_{ij}(u) \circ p_{ij}(v) \circ p_{ij}(u^{-1} v^{-1}), \text{ where }
   p_{ij}(x)=(x[i], x[i]) \circ (x^{-1}[i], x^{-1}[j]).\] 
\end{lemma}
\begin{proof}
Fix a vector $g\in G^n$ and denote by $\gamma_{ij}(h, x)$ the path of length $4$ in $W$ which connects 
the following five points of $W_0$:
%the points $\left(\begin{smallmatrix} x_i \\ n_i \end{smallmatrix}\right)$, $i=0..4$ whose components are given as follows:
\[
...
One can easily check that the image $\gamma_{ij}(h, x)$ in $S$ under $\pi_N g$ is precisely the element $h_{ij}(x)$, while
$\pi_{H\times H} f$ sends $\gamma_{ij}(h, x)$ to the loop $p_{ij}(x)$ (here $f, g$ are the maps from the proof of~\cref{lm:quillen-a}).
\end{proof}

%The isomorphism of \cref{lm:quillen-a} can be described on the level of generators concrete as follows.
...
If we denote by $K$ the kernel of the map $\nu\colon \pi_1(T) \to G^{2n}$ induced by the embedding $h_n^{-1}(0)=T \subseteq BG^{2n}$ we come to the following commutative diagram:
...
We are going to compute the images of generators $c_{ij}(x, y) = x \wedge y \in \HH_2(G, \ZZ)$ under $\alpha$ and $\varphi\beta$.
In order to do this we need to have a better description of the set of 1-simplices of $F_{h_n}$.
...
It will be convenient for us to perform all calculations inside the subset $F'\subset (F_{h_n})_1$ consisting of those triples for which $t'=0$ 
 (thus we can identify its elements with pairs $(g, t)$).

It is obvious that $F_{h_n}$ is fibrant. 
We want to find a concrete formula for the element of $F'$ homotopy equivalent to the concatenation of two elements $(g_1, t_1), (g_2, t_2) \in F'$.
It is easy to see that the element $(g_1g_2, t)$, where $t$ is the $1$-st face of the filler $f$ for the $3$-horn $(t_2, ,t_1, h_n(g_1, g_2))$, is the desired one.
...
Substituting concrete values of $p_i$ and using the fact that $d_1t_2=0$ we get the following expression for $t$:
\begin{multline} t = d_1f = p_0 - s_1 d_2 p_0 - s_0 d_1 p_0 + s_0 d_2 p_0 + p_2 - p_3 + s_1 d_1 p_3 = \\
...
\begin{equation} \label{eq:normalized} c' = c - s_0d_0c - s_1d_1c + s_1d_0 = (x, y) - (y, x) - (x, e) +(y, e) + (e, x) - (e, y). \end{equation}
 
Since $F_{h_n}$ and $\ZZ[BG]$ are fibrant, the elements of $\pi_1(F_{h_n}, 0)$ correspond to
 classes of pairs $(g, t) \in G^{2n} \times \ZZ[BG]_2$ satisfying $d_2(t) = h_n(g)$. 
...
$((e, d_{ij}(x)), t(x)),$ where $t(x)=-(x, x^{-1}) + (e, e) - (x^{-1}, e) + (e, x^{-1})$. Notice that $d_1t(x)=d_0t(x)=0$.
 
It is not hard to describe the group structure of $\pi_1(F_{h_n}, 0)$.
Using the above formula we get that the image of $c_{ij}(u,v)$ via ??? equals $((e, c_{ij}(x, y)), t)$, where
%t = t(x) + t(y) + t(x^{-1}y^{-1}) - h_n(g_1, g_2) - h_n(g_1g_2, g_3) - s_1h_n(g_2) + s_0h_n(g_2) - s_1h_n(g_3) + s_1h_n(g_3) + s_1h_n(g_1g_2) + s_1h_n(g_1g_2g_3)
\begin{multline} t = 2(e, e)-(x^{-1}, e) +(xy, x^{-1}y^{-1}) -(xy, e) -(x, x^{-1}) -(y, y^{-1}) -\\  -(e, x^{-1}y^{-1}) +(e, x^{-1}) +(x, y) +(x^{-1}, y^{-1}) -(e, y) -(xyx^{-1}y^{-1}, e) +(y, e).\end{multline}
...
\[s_1 = \sum_{i=1}^{2n}(g^0_i + g_i, -g_i) - \sum_j (a_j + b_j, -b_j).\] \end{comment}
...
\begin{proof}
 ...
 \begin{itemize}
  ...
  \item Using \cref{item:continue} we extend the homotopy $a |\delta^q| \cong a'|\delta^q| \cong |a''|$ to a homotopy of diagrams $(a|\delta^q|, A|\delta^q|) \cong (|a''|, A')$.
