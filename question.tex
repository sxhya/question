...
for which $i \in C$ there exists a map $\widetilde{F}$ so that the lower triangle commutes strictly and the upper one commutes up to a fiberwise homotopy $H$
...
The map $h \colon X^\infty \to \ZZ[X]$ satisfies the weak covering homotopy property with respect to the class $C_{pr}$.
\end{lemma}
\begin{proof}
 Notice first that the problem can be reduced to the special case $g=0$.
 Indeed, since $U$ is finite the image of $g$ is contained in $X^{2N} \subset X^\infty$.
 If we could find a lift $\widetilde{F}_0$ for a pair of maps $(g=0, F - hgr)$ then $\widetilde{F} = gr \oplus \widetilde{F}_0[N]$
...
\begin{lemlist}
 ...
 \item \label{item:weaker} The third assertion remains true if instead of the strict equality $\widetilde{A} i_0 = a$ 
       we require that $\widetilde{A}i_0$ and $a$ are fiberwise homotopic.
\end{lemlist}
\end{lemma}
...
\begin{proof}
 ...
 \begin{itemize}
  ...
  \item It is clear that there exists a map $a'\colon D^k\to |E|$ that extends $|a'_0|$ and is fiberwise homotopic to $a$.
