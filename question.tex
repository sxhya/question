...
Since $F_{h_n}$ and $\ZZ[BG]$ are fibrant, the elements of $\pi_1(F_{h_n}, 0)$ can be described as
 classes of pairs $(g, s) \in G^{2n} \times \ZZ[BG]_2$ satisfying $d_2(s) = h_n(g)$ up to natural homotopy relation. 
...
Indeed, if $[(g^1, s^1)]$ and $[(g^2, s^2)]$ are two elements of $\pi_1(F_{h_n}, 0)$, then their product is represented by the pair $(g^1g^2, s)$, where
 $s$ is the $1$-st face of the filler $f$ for the $3$-horn $(s^2, ,s^1, h_n(g^1, g^2))$.

Now if $(p_0, ,p_2, p_3)$ is a horn then by Moore's theorem we get the following formula for the filler:
 $f = s_0 p_0 - s_2 s_0 d_2 p_0 - s_1 s_0 d_1 p_0 + s_1 s_0 d_2 p_0 + s_1 p_2 + s_2 p_3 - s_1 p_3$. 
Substituting  $p_0 + p_2 - p_3 - s_1 d_2 p_0  + s_1 d_1 p_3 + s_0 d_2 p_0$
