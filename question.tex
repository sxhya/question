...
\title{A note on amalgams of symmetric groups and $\mathfrak{H}_n$-extensions}
...
By definition, an {\it $\mathfrak{H}_n$-extensions} is an extension $H$ of the group $D_n(G)$ that fits into the following diagram of groups.
\[ \xymatrix{ G \mathbin{\widetilde{\wedge}} G \ar@{->>}[d] \ar@{->>}[rd] & \\ U_H \pullbackcorner \ar@{^{(}->}[d] \ar@{->>}[r] & [G, G] \ar@{^{(}->}[d]^{\iota_k} \\ H \ar@{->>}[r] & D_n(G)} \]
