...
Thus, we conclude that every generator $c_{ij}(u, v)$ of $\Ker(\pi_1(S)\to N)$ corresponds to the loop $p_{ij}(u) \circ p_{ij}(v) \circ p_{ij}(u^{-1} v^{-1})$ in $T$.
...
It is easy to check that the image of the loop $p_{ij}(x)$ under the natural map $T \to F_{h_n}$ corresponds to the class of
$((e, d_{ij}(x)), (x, x^{-1}) + (e, e) - (x, e) - (e, x^{-1})). $
...
By Moore's theorem filler for any horn $(p_0, ,p_2, p_3)$ in a simplicial abelian group can be computed via the formula
 $f = s_0 p_0 - s_2 s_0 d_2 p_0 - s_1 s_0 d_1 p_0 + s_1 s_0 d_2 p_0 + s_1 p_2 + s_2 p_3 - s_1 p_3$. 
Substituting concrete values of $p_i$ and using the fact that $d_1t_2$ is zero we get the following formula for $t$:
\begin{multline} t = d_1f = p_0 - s_1 d_2 p_0 - s_0 d_1 p_0 + s_0 d_2 p_0 + p_2 - p_3 + s_1 d_1 p_3 = \\
             = t_2 - s_1 h_n(g_2) + s_0 h_n(g_2) + t_1 - h_n(g_1, g_2) + s_1 h_n (g_1g_2). \end{multline}
