...
\begin{lemma} Let $U$ be a compact and $V$ a polyhedral simplicial sets and
 let $\iota \colon U \hookrightarrow V$ be a map between them that is a trivial cofibration and
 admits a retract $r \colon V \to U$ (i.\,e. $ri=id_U$). Let $X$ be a fibrant connected simplicial set and let $F$, $g$ be arbitrary maps
 such that outer rectangle in the diagram below commutes strictly.
\begin{equation} \nonumber \xymatrix{ 
...
\begin{proof}
 ...
 If we could find a lift $\widetiled{F}_0$ for a pair of maps $(g=0, F_0)$ then $\widetilde{F} = gr \oplus \widetilde{F}_0[N]$
