...
We would like to have a more concrete description of the isomorphism of~\cref{cor:ker-iso}.
Fix a vector $g\in G^n$ and denote by $\gamma_{ij}(h, x)$ the path of length $4$ in $W$ connecting the points 
 $\left(\begin{smallmatrix} x_i \\ n_i \end{smallmatrix}\right)$, $i=0..4$ whose components are given as follows:
\[\begin{array}{ccccc}
   x_0 = e, & x_1 = x[i], & x_2 = x[i], & x_3 = e, & x_4 = e; \\
   n_0 = (h; e), & n_1 = (h, e), & n_2 = (d_{ij}(x) h, (ij)), & n_3 = (d_{ij}(x) h, (ij)), & n_4 = (d_{ij}(x) h, e). \end{array}\]
One can easily check that the image $\gamma_{ij}(h, x)$ in $S$ under $\pi_N g$ is precisely the element $h_{ij}(x)$ while
$\pi_{H\times H} f$ sends $\gamma_{ij}(h, x)$ to the loop $p_{ij}(x) = (x[i], x[i]) \circ (x^{-1}[i], x^{-1}[j])$.
Thus, we conclude that the element $c_{ij}(u, v)$ of $\pi_1(S)$ corresponds to the loop $p_{ij}(u) \circ p_{ij}(v) \circ p_{ij}(u^{-1} v^{-1})$ in $T$.
...
Indeed, if $[(g_1, t_1)]$ and $[(g_2, s_2)]$ are two elements of $\pi_1(F_{h_n}, 0)$, then their product is represented by the pair $(g_1g_2, t)$, where
 $t$ is the $1$-st face of the filler $f$ for the $3$-horn $(t_2, ,t_1, h_n(g_1, g_2))$.
